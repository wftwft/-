...
exports.main = async (event, context) => {
  ...
  for (let i = 0; i < 7; i++) {
    week[i][1] = {
      title: data[3][i + 2] ? data[3][i + 2].replace(/\(.*?\)/, "") : undefined,
      classroom: data[4][i + 2],
      course: parseFn(data[5][i + 2])
    }
    week[i][2] = {
      title: data[11][i + 2] ? data[11][i + 2].replace(/\(.*?\)/, "") : undefined,
      classroom: data[12][i + 2],
      course: parseFn(data[13][i + 2])
    }
    week[i][3] = {
      title: data[20][i + 2] ? data[20][i + 2].replace(/\(.*?\)/, "") : undefined,
      classroom: data[21][i + 2],
      course: parseFn(data[22][i + 2])
    }
    week[i][4] = {
      title: data[28][i + 2] ? data[28][i + 2].replace(/\(.*?\)/, "") : undefined,
      classroom: data[29][i + 2],
      course: parseFn(data[30][i + 2])
    }
    week[i][5] = {
      title: data[37][i + 2] ? data[37][i + 2].replace(/\(.*?\)/, "") : undefined,
